...
# Output: $WORK/ov3 (the fetched packages' own /nix tree) and $WORK/ov4 (session files: systemd units,
...
"$GOGET" install --root "$WORK/nix-tmp" --system -y \
    ...
    glibc-locales fcitx5 noto-fonts-cjk-sans polkit
...
echo "== session files (systemd units, wrapper scripts, configs)"
